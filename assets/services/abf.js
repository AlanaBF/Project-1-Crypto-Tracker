...
$(".card-link").on("click", function (event) {
    ...
    var linkClick = $("favourites").val().trim();
    saveToLs(linkClick);
    renderFavourites();
});
...
function renderFavourites() {
    ...
    for (var i=0; favourites.length; i++) {
        var favouritesHistory = $("<button>");
        favouritesHistory.addClass("favourites");
        favouritesHistory.attr("data-name", favourites[i]);
        favouritesHistory.text(favourites[i])
        // add id # into html favourites file
        $("").append(favouritesHistory);
    }
}
